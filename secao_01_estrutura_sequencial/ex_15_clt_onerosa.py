...
def calcular_assalto_no_salario():
    ...
    salario = float(input('Informe o valor/hora do seu salário R$: '))
    horas_trabalhadas = float(input('Informe as horas trabalhadas no mês: '))
    salario_bruto = salario*horas_trabalhadas
    ir = (salario*horas_trabalhadas)*0.11
    inss = (salario*horas_trabalhadas)*0.08
    sindicato = (salario*horas_trabalhadas)*0.05
