...
def calcular_preco_da_compra(kilos_de_morango: int, kilos_de_maca: int):
    ...
    if kilos_de_maca <= 5:
        preco_kg_maca = 1.80
        valor_total_maca = kilos_de_maca * preco_kg_maca
    else:
        preco_kg_maca = 1.50
        valor_total_maca = kilos_de_maca * preco_kg_maca
    ...
    if kilos_de_morango <= 5:
        ...
        valor_total_morango = kilos_de_morango * preco_kg_morango
    else:
        preco_kg_morango = 2.20
        valor_total_morango = kilos_de_morango * preco_kg_morango

    kg_total_frutas = kilos_de_morango + kilos_de_maca
    total_compra = valor_total_maca + valor_total_morango
    desconto = 0

    if kg_total_frutas >= 8 or total_compra > 25:
        desconto = total_compra * 0.10
        total_compra -= desconto

    if kilos_de_maca == 0:
        print(f'(+)  Morango  - valor:  R$  {valor_total_morango:>4.2f} - quantidade:  {kilos_de_morango} kg - preço: R$ {preco_kg_morango:.2f}/kg')
        print(f'(-)  Desconto - valor:  R$  {desconto:>3.2f}')
        print(f'          Valor Total:  R$  {total_compra:>4.2f}')
    elif kilos_de_morango == 0:
        print(f'(+)  Maça     - valor:  R$  {valor_total_maca:>4.2f} - quantidade:  {kilos_de_maca} kg - preço: R$ {preco_kg_maca:.2f}/kg')
        print(f'(-)  Desconto - valor:  R$  {desconto:>3.2f}')
        print(f'          Valor Total:  R$  {total_compra:>4.2f}')
    else:
        print(f'(+)  Morango  - valor:  R$  {valor_total_morango:>4.2f} - quantidade:  {kilos_de_morango} kg - preço: R$ {preco_kg_morango:.2f}/kg')
